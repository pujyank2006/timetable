...
def post_availability_service():
    ...
    try:
        ...
        slots_list = list(dict.fromkeys([int(s) for s in availability_data.get('current_unavailability', [])]))

    except Exception:

        return jsonify({"error": "current_unavailability must be a list of integers"}), 400
    ...
    # 2. Store teacher's original unavailability as authentic_unavailability

    # and initially set current_unavailability to the same value

    availability_data['authentic_unavailability'] = slots_list

    availability_data['current_unavailability'] = slots_list
